...
void update_debug_strings()
{
    ...
    snprintf(strPlayerChunk, 32,        "CHUNK: %d %d",             (i16)floorf(lily.pos.x / CHUNK_DIAMETER), (i16)floorf(lily.pos.y / CHUNK_DIAMETER));
    snprintf(strPlayerDirection, 32,    "YAW: %.1f PITCH: %.1f",    lily.yaw, lily.pitch);
    snprintf(strBlockCount, 32,         "BLOCKS: %lld",             world_stats.blockCount);
    snprintf(strQuadCount, 32,          "QUADS: %lld",              world_stats.quadCount);
    snprintf(strTriCount, 32,           "TRIS: %lld",               world_stats.quadCount * 2);
    snprintf(strVertexCount, 32,        "VERTICES: %lld",           world_stats.quadCount * 6);
}
