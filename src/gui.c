...
Rectangle	container_inventory_survival =			{0, 0, 176, 184};
...
v2i16		container_inventory_survival_position;
v2i16		container_inventory_survival_first_slot_position;
...
// ---- game menu --------------------------------------------------------------
u8			button_state_back_to_game =				BUTTON_LISTENING;
u8			button_state_options =					BUTTON_LISTENING;
u8			button_state_save_and_quit_to_title =	BUTTON_LISTENING;
        ...
        else
        {
			if ((codepoint != ' ') && (codepoint != '\t'))
			{
				DrawTextCodepoint(font_bold, codepoint,
						...
						font_size, ColorAlpha(ColorBrightness(tint, -0.5f), 0.4f));
...
void draw_text_centered(Font font, const str *str, v2i16 pos, f32 font_size, f32 spacing, Color tint, u8 center_vertically)
{
	...
	if (center_vertically) pos.y -= (font_size/1.8f);
...
void draw_texture(Texture2D texture, Rectangle source, v2i16 pos, v2i16 scl, Color tint) /* scale is based on source.scale*scl */
{
	...
	Vector2 topRight =		(Vector2){pos.x + (source.width*scl.x),	pos.y};
	Vector2 bottomLeft =	(Vector2){pos.x,						pos.y + (source.height*scl.x)};
...
void draw_texture_simple(Texture2D texture, Rectangle source, v2i16 pos, v2i16 scl, Color tint) /* scale is based on scl */
{
	...
	Vector2 topLeft =		(Vector2){pos.x,			pos.y};
	Vector2 topRight =		(Vector2){pos.x + scl.x,	pos.y};
	Vector2 bottomLeft =	(Vector2){pos.x,			pos.y + scl.y};
...
void draw_game_menu()
{
	...
	draw_button(texture_hud_widgets, button,
			...
			&button_state_save_and_quit_to_title,
...
void draw_hud()
{
	...
	container_inventory_survival_position = (v2i16){
		roundf((win.scl.x/2) - ((f32)container_inventory_survival.width/2)),
		roundf((win.scl.y/2) - ((f32)container_inventory_survival.height/2)),
	};
...
void draw_inventory_survival()
{
	...
	draw_texture(texture_container_inventory,
			container_inventory_survival,
			container_inventory_survival_position, 
			(v2i16){
			setting.gui_scale,
			setting.gui_scale}, COL_TEXTURE_DEFAULT);
