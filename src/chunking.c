...
chunking_init(void)
{
    ...
    v3i32 center = {0};
    v3i32 coordinates = {0};
    u32 distance[CHUNK_BUF_VOLUME_MAX] = {0};
    u32 index[CHUNK_BUF_VOLUME_MAX] = {0};
    ...
    {
        CHUNK_QUEUE.size_1 = CHUNKS_MAX[SET_RENDER_DISTANCE];
    }
    else if (CHUNKS_MAX[SET_RENDER_DISTANCE] <
            CHUNK_QUEUE_1ST_MAX + CHUNK_QUEUE_2ND_MAX)
    {
        CHUNK_QUEUE.size_1 = CHUNK_QUEUE_1ST_MAX;
        CHUNK_QUEUE.size_2 =
    ...
    {
        CHUNK_QUEUE.size_1 = CHUNK_QUEUE_1ST_MAX;
        CHUNK_QUEUE.size_2 = CHUNK_QUEUE_2ND_MAX;
        CHUNK_QUEUE.size_3 =
    ...
    if (mem_map((void*)&CHUNK_QUEUE.priority_1,
                ...
                "chunking_init().CHUNK_QUEUE.priority_1") != ERR_SUCCESS)
        goto cleanup;

    if (mem_map((void*)&CHUNK_QUEUE.priority_2,
                CHUNK_QUEUE_2ND_MAX * sizeof(Chunk**),
                "chunking_init().CHUNK_QUEUE.priority_2") != ERR_SUCCESS)
        goto cleanup;

    if (mem_map((void*)&CHUNK_QUEUE.priority_3,
                CHUNK_QUEUE_3RD_MAX * sizeof(Chunk**),
                "chunking_init().CHUNK_QUEUE.priority_3") != ERR_SUCCESS)
        goto cleanup;
...
{
    u32 i = 0;
    for (; i < CHUNKS_MAX[SET_RENDER_DISTANCE]; ++i)
    {
...
chunking_free(void)
{
    ...
    mem_unmap((void*)&CHUNK_QUEUE.priority_1,
            CHUNK_QUEUE_1ST_MAX * sizeof(Chunk**),
            "chunking_free().CHUNK_QUEUE.priority_1");

    mem_unmap((void*)&CHUNK_QUEUE.priority_2,
            CHUNK_QUEUE_2ND_MAX * sizeof(Chunk**),
            "chunking_free().CHUNK_QUEUE.priority_2");

    mem_unmap((void*)&CHUNK_QUEUE.priority_3,
            CHUNK_QUEUE_3RD_MAX * sizeof(Chunk**),
            "chunking_free().CHUNK_QUEUE.priority_3");
}
...
chunk_tab_shift(v3i16 player_chunk, v3i16 *player_delta_chunk)
{
    ...
    for (i = 0; i < CHUNK_BUF_VOLUME; ++i)
    {
        ...
        _mirror_index =
        (v3u32){
            i + CHUNK_BUF_DIAMETER - 1 - (_coordinates.x * 2),

            (_coordinates.z * CHUNK_BUF_LAYER) +
                ((CHUNK_BUF_DIAMETER - 1 - _coordinates.y) *
                 CHUNK_BUF_DIAMETER) + _coordinates.x,

            ((CHUNK_BUF_DIAMETER - 1 - _coordinates.z) * CHUNK_BUF_LAYER) +
                (_coordinates.y * CHUNK_BUF_DIAMETER) + _coordinates.x,
        };
        ...
        switch (INCREMENT)
        {
            case -1:
                _is_on_edge =
                    (v3u8){
                        (_coordinates.x == CHUNK_BUF_DIAMETER - 1) ||
                            (chunk_tab[i + 1] == NULL),

                        (_coordinates.y == CHUNK_BUF_DIAMETER - 1) ||
                            (chunk_tab[i + CHUNK_BUF_DIAMETER] == NULL),

                        (_coordinates.z == CHUNK_BUF_DIAMETER - 1) ||
        ...
        switch (INCREMENT)
        {
            case -1:
                _target_index = (v3u32){
                        ...
                        (_coordinates.y == 0) ? i : i - CHUNK_BUF_DIAMETER,
                        (_coordinates.z == 0) ? i : i - CHUNK_BUF_LAYER};
                break;

            case 1:
                _target_index = (v3u32){
                        (_coordinates.x == CHUNK_BUF_DIAMETER - 1) ? i : i + 1,
