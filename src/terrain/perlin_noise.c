...
{
    f32 g = RAND_TAB[(seed + RAND_CONST_0 + x) % RAND_TAB_VOLUME];
    return (v - x) * g;
}

f32 gradient_2d(f32 vx, f32 vy, i32 x, i32 y, u64 seed)
{
    f32 gx = RAND_TAB[(seed + (RAND_CONST_1 + x) * (RAND_CONST_2 + y)) % RAND_TAB_VOLUME];
    f32 gy = RAND_TAB[(seed + (RAND_CONST_3 + y) * (RAND_CONST_4 + x)) % RAND_TAB_VOLUME];
    return (vx - x) * gx + (vy - y) * gy;
}

f32 gradient_3d(f32 vx, f32 vy, f32 vz, i32 x, i32 y, i32 z, u64 seed)
{
    f32 gx = RAND_TAB[(seed + (RAND_CONST_5 + x) * (RAND_CONST_6 + y) * (RAND_CONST_7 + z)) % RAND_TAB_VOLUME];
    f32 gy = RAND_TAB[(seed + (RAND_CONST_8 + y) * (RAND_CONST_9 + z) * (RAND_CONST_10 + x)) % RAND_TAB_VOLUME];
    f32 gz = RAND_TAB[(seed + (RAND_CONST_11 + z) * (RAND_CONST_12 + x) * (RAND_CONST_13 + y)) % RAND_TAB_VOLUME];
        ...
        u32 octaves, f32 amplitude_persistence, f32 frequency_persistence, u64 seed)
{
    ...
    while (octaves--)
    {
        result += perlin_noise_1d(x, amplitude, frequency, seed);
...
f32 perlin_noise_2d(i32 x, i32 y, f32 amplitude, f32 frequency, u64 seed)
{
    ...
    g0 = gradient_2d(vx, vy, ax, ay, seed);
    g1 = gradient_2d(vx, vy, bx, ay, seed);
        ...
        u32 octaves, f32 amplitude_persistence, f32 frequency_persistence, u64 seed)
{
    ...
    while (octaves--)
    {
        result += perlin_noise_2d(x, y, amplitude, frequency, seed);
        amplitude *= amplitude_persistence;
        frequency *= frequency_persistence;
    }
    return result;
...
f32 perlin_noise_3d(i32 x, i32 y, i32 z, f32 amplitude, f32 frequency, u64 seed)
{
    ...
    g0 = gradient_3d(vx, vy, vz, ax, ay, az, seed);
    g1 = gradient_3d(vx, vy, vz, bx, ay, az, seed);
    l0 = fsl_lerp_cubic_f32(g0, g1, dx);
        ...
        u32 octaves, f32 amplitude_persistence, f32 frequency_persistence, u64 seed)
{
    ...
    while (octaves--)
    {
        result += perlin_noise_3d(x, y, z, amplitude, frequency, seed);
        amplitude *= amplitude_persistence;
        frequency *= frequency_persistence;
    }
    return result;
